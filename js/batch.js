...
window.wholeSegmentMarkComplete = async function(segmentNumber) {
    ...
    map.closePopup();
    try {
        const r = await (await apiCall('updateWholeSegment', { segmentNumber: encodeURIComponent(segmentNumber), statusString: Array(numSmallSegs }).json();
...
async function _refreshProgress() {
    const d = await (await apiCall('getProgress').json();
    if (d.segments) {
