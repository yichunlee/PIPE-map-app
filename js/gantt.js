...
<body><div class="spinner"></div><p style="color:#666;font-size:14px;">載入甘特圖資料中...</p></body></html>`;
    ...
    try {
        ...
        const [result, upResult] = await Promise.all([
            apiCall('getGanttItems')
            apiCall('getUnitPrices', { projectName: encodeURIComponent(projName)).then(r => r.json() })
        ]);
...
async function loadGanttData() {
    try {
        const [res1, res2] = await Promise.all([
            apiCall('getGanttItems')
            apiCall('getUnitPrices', { projectName: currentProject ? currentProject.name || '' : '' })
        ]);
