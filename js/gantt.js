...
<body><div class="spinner"></div><p style="color:#666;font-size:14px;">載入甘特圖資料中...</p></body></html>`;
    ...
    try {
        ...
        const [result, upResult] = await Promise.all([
            fetch(API_URL + '?action=getGanttItems&pipelineId=' + encodeURIComponent(currentPipeline.id)).then(r => r.json()),
            fetch(API_URL + '?action=getUnitPrices&pipelineId=' + encodeURIComponent(currentPipeline.id) + '&projectName=' + encodeURIComponent(projName)).then(r => r.json())
        ]);
...
return el ? el.getBoundingClientRect().width || el.offsetWidth : 1;
    ...
    function pxToDays(px) { return px / innerWidth() * totalRangeMs / 86400000; }
...
tooltip.id = 'ganttDragTip';
tooltip.style.cssText = 'display:none;position:fixed;background:rgba(0,0,0,0.75);color:white;font-size:11px;padding:4px 10px;border-radius:5px;pointer-events:none;z-index:9999;white-space:nowrap;';
...
async function loadGanttData() {
    try {
        const [res1, res2] = await Promise.all([
            fetch(API_URL + '?action=getGanttItems&pipelineId=' + encodeURIComponent(currentPipeline.id)),
            fetch(API_URL + '?action=getUnitPrices&pipelineId=' + encodeURIComponent(currentPipeline.id) + '&projectName=' + encodeURIComponent(currentProject ? currentProject.name || '' : ''))
        ]);
        const result = await res1.json();
        const upResult = await res2.json();
...
async function saveGanttNew() {
    ...
    const qs = '?action=addGanttItem&pipelineId=' + encodeURIComponent(currentPipeline.id) +
        ...
        '&unitPrice=' + encodeURIComponent(data.unitPrice || '');
    try {
