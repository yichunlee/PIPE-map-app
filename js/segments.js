...
async function loadSegmentData() {
    try {
        const result = await apiCall('getSegments');
...
async function confirmAddSegment() {
    ...
    try {
        const result = await apiCall('addSegment', { pipelineId: currentPipeline.id, diameter: diameter, pipeType: pipeType, method: method, notes: notes, segmentNumber: nextSegNum, endDistance: endDist });
...
async function confirmEditSegment(segmentNumber) {
    ...
    try {
        const result = await apiCall('updateSegment', { pipelineId: currentPipeline.id, segmentNumber: segmentNumber, diameter: diameter, pipeType: pipeType, method: method, notes: notes, startDistance: Math.round(startDist) });
...
window.confirmBatchSegments = async function(segments) {
    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        try {
            const result = await apiCall('addSegment', { pipelineId: currentPipeline.id, segmentNumber: segmentNumber, diameter: diameter, pipeType: pipeType, method: method, notes: notes, startDistance: Math.round(seg.start) });
