...
async function loadSegmentData() {
    try {
        const response = await fetch(API_URL + '?action=getSegments&pipelineId=' + encodeURIComponent(currentPipeline.id));
...
async function confirmEditSegment(segmentNumber) {
    ...
    try {
        const url = API_URL + 
            ...
            '&pipelineId=' + encodeURIComponent(currentPipeline.id) +
            '&segmentNumber=' + encodeURIComponent(segmentNumber) +
...
window.confirmBatchSegments = async function(segments) {
            ...
            for (const seg of segmentsToDelete) {
                try {
                    const deleteUrl = API_URL + 
                        '?action=deleteSegment' +
                        '&pipelineId=' + encodeURIComponent(currentPipeline.id) +
                        '&segmentNumber=' + encodeURIComponent(seg.segmentNumber);
                    
                    console.log(`   刪除段落: ${seg.segmentNumber}, URL: ${deleteUrl}`);
    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        try {
            const url = API_URL + 
                ...
                '&pipelineId=' + encodeURIComponent(currentPipeline.id) +
                '&segmentNumber=' + encodeURIComponent(segmentNumber) +
                '&startDistance=' + Math.round(seg.start) +
                '&endDistance=' + Math.round(seg.end) +
                '&diameter=' + encodeURIComponent(diameter) +
                '&pipeType=' + encodeURIComponent(pipeType) +
                '&method=' + encodeURIComponent(method) +
                '&notes=' + encodeURIComponent(notes);
    ...
    const progressResponse = await fetch(API_URL + '?action=getProgress&pipelineId=' + encodeURIComponent(currentPipeline.id));
