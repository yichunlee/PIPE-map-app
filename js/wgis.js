...
async function toggleWgisDataset(fileId, fileName) {
    ...
    if (!ds.loaded) {
        ...
        try {
            const data = await apiCall('getWgisFile', { fileId: fileId });
            if (!data.success) throw new Error(data.error);
            // base64 → bytes → text (自動偵測 UTF-8 / Big5)
            const binaryStr = atob(data.data);
            const bytes = new Uint8Array(binaryStr.length);
            for (let i = 0; i < binaryStr.length; i++) bytes[i] = binaryStr.charCodeAt(i);
            // UTF-8 BOM 偵測 (EF BB BF)
            let text;
            if (bytes[0] === 0xEF && bytes[1] === 0xBB && bytes[2] === 0xBF) {
                text = new TextDecoder('utf-8').decode(bytes);
            } else {
                // 嘗試 UTF-8，若有亂碼退回 Big5
                const utf8 = new TextDecoder('utf-8', { fatal: true });
                try { text = utf8.decode(bytes); } catch(e) {
...
async function deleteWgisCloudFile(fileId, fileName) {
    ...
    try {
        ...
        const data = await apiCall('deleteWgisFile', { fileId: fileId, email: currentUser ? currentUser.email : '' });
        if (!data.success) throw new Error(data.error);
...
function parseWgisCsv(text) {
    ...
    const ri  = headers.findIndex(h => h.includes('WGS84路徑'));
    ...
    if (ri < 0 && (fx1<0||fy1<0||fx2<0||fy2<0)) throw new Error('找不到 TWD97 座標欄位');
    const result = [];
    for (let i = 1; i < lines.length; i++) {
        try {
            const cols = parseCSVLine(lines[i]);
            let coords;
            // 優先使用 WGS84路徑 欄位（沿路轉換後的多點座標）
            if (ri >= 0 && cols[ri] && cols[ri].includes('|')) {
                coords = cols[ri].split('|').map(p => {
            ...
            result.push({ coords, d:di>=0?cols[di]||'':'', m:mi>=0?cols[mi]||'':'', l:li>=0?cols[li]||'':'' });
        } catch(e) {}
    }
    return result;
}
