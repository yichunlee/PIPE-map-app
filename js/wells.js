...
window.loadShafts = async function() {
    try {
        const result = await apiCall('getShafts', { pipelineId: currentPipeline.id });
        if (result.shafts) {
...
window.saveNewShaft = async function(lat, lng) {
    ...
    try {
        await apiCall('addShaft', {
                pipelineId: payload.pipelineId, lat: lat, lng: lng,
                name: payload.name, type: payload.type,
                designDepth: payload.designDepth, currentDepth: payload.currentDepth,
                status: payload.status, notes: payload.notes, creator: payload.creator
            }, { errorPrefix: '新增失敗' });
            map.closePopup();
            await loadShafts();
...
window.saveEditShaft = async function(shaftId) {
    ...
    if (!name) { showToast('請輸入名稱', 'warning'); return; }
    try {
        await apiCall('updateShaft', {
            shaftId: shaftId, name: name,
            type: document.getElementById('sf_type').value,
            designDepth: document.getElementById('sf_designDepth').value || '0',
            currentDepth: document.getElementById('sf_currentDepth').value || '0',
            status: document.getElementById('sf_status').value,
...
window.deleteShaft = async function(shaftId) {
    ...
    try {
        const result = await apiCall('deleteShaft', { shaftId: shaftId });
        if (result.success) {
            map.closePopup();
            await loadShafts();
