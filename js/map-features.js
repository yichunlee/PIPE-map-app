...
window.deleteMapNote = async function(noteId) {
    ...
    try {
        const result = await apiCall('deleteMapNote', { noteId: noteId });
...
window.saveEditNote = async function(noteId, isNode) {
    ...
    try {
        const result = await apiCall('updateMapNote', { noteId: noteId, text: newText });
...
window.showRightClickMenu = function(latlng, clientX, clientY) {
    ...
    if (!currentPipeline) {
        ...
        menu.innerHTML =
            ...
            '<div class="rcm-item" style="border-top:1px solid #f0f0f0;background:#f7fbff;" onclick="closeRightClickMenu();startAttrsRectSelect()">📏 <span>圈選設定屬性</span></div>' +
...
async function loadGanttItemsForLabels() {
    ...
    try {
        const result = await apiCall('getGanttItems', { pipelineId: currentPipeline.id });
