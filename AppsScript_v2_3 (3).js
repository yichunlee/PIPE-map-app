  ...
  try {
    switch(action) {
      ...
      case 'getMapNotes':
        ...
        break;
      case 'addMapNote':
        result = addMapNote(params.lat, params.lng, params.text, params.creator, params.photo);
        break;
      case 'updateMapNote':
...
function addMapNote(lat, lng, text, creator, photo) {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    ...
    if (!sheet) {
      ...
      sheet.appendRow(['備註ID', '經度', '緯度', '備註內容', '建立時間', '建立者', '照片']);
    }
    ...
    sheet.appendRow([noteId, lng, lat, text, timestamp, creator, photoUrl]);
