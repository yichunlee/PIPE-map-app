// Google Apps Script for 管線施工進度管理系統 v2.3
// 支援 Google OAuth2 登入 + 開放註冊制
// 任何人都能用 Google 帳號登入,系統自動記錄到「使用者清單」工作表
  ...
  if (e && e.postData && e.postData.contents) {
    try {
      ...
      if (contentType.includes('application/json')) {
        ...
        // Form-urlencoded 格式：e.parameter 只含 URL query string，POST body 需手動解析
        Logger.log('✅ 使用 form-urlencoded 格式，手動解析 POST body');
        ...
        bodyStr.split('&').forEach(function(pair) {
          ...
          if (idx > -1) {
            ...
            if (!params[key]) params[key] = val; // query string 優先
          }
        });
        if (!action) action = params.action;
      }
...
function verifyUser(params) {
  try {
    ...
    // 取得試算表
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    let userSheet = ss.getSheetByName('使用者清單');
    
    // 如果「使用者清單」工作表不存在，自動建立
    if (!userSheet) {
      userSheet = ss.insertSheet('使用者清單');
      // 設定標題列
      userSheet.appendRow(['Email', '角色', '姓名', '頭像網址', '首次登入時間', '最後登入時間', '狀態', '備註']);
      // 設定標題列樣式
    ...
    for (let i = 1; i < data.length; i++) {
      if (data[i][0] === email) {
        userRow = i + 1; // +1 因為 Google Sheets 從 1 開始
        userRole = data[i][1] || 'user';
        userStatus = data[i][6] || '啟用';
        break;
      }
    }
    ...
    if (userRow === -1) {
      ...
      Logger.log('✅ 新使用者自動註冊: ' + email + ' (' + name + ')');
      isNewUser = true;
      userRole = 'user';
      userStatus = '啟用';
    } else {
      // 更新最後登入時間
      userSheet.getRange(userRow, 6).setValue(now);
      Logger.log('✅ 使用者登入: ' + email + ' (角色: ' + userRole + ')');
    }
    
    // 檢查帳號是否被停用
    if (userStatus === '停用' || userStatus === '禁用') {
      Logger.log('⛔ 已停用的帳號嘗試登入: ' + email);
      return {
        success: true,
        authorized: false,
        email: email,
    ...
    Logger.log('verifyUser Error: ' + error.toString());
    return {
      success: false,
      authorized: false,
      error: error.toString()
