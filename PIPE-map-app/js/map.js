async function showPipelineDetail(pipelineId, keepView = false) {
    ...
    if (!currentPipeline._progressLoaded) {
        showLoading(true);
        try {
            const progressRes = await fetch(API_URL + '?action=getProgress&pipelineId=' + encodeURIComponent(pipelineId));
                            ...
                            if (segment.nodeRange && segment.nodeRange.trim()) {
                                ...
                                if (nodePos) {
                                    ...
                                    nm.on('click', async function() {
                                        ...
                                        segment.notes = notes;
                                        try {
                                            await fetch(API_URL + '?action=updateSegment' +
                                                '&pipelineId=' + encodeURIComponent(currentPipeline.id) +
                                                '&segmentNumber=' + encodeURIComponent(segment.segmentNumber) +
                                                '&startDistance=' + segment.startDistance +
                                                '&endDistance=' + segment.endDistance +
                                                '&diameter=' + encodeURIComponent(segment.diameter || '') +
                                                '&pipeType=' + encodeURIComponent(segment.pipeType || '') +
                                                '&method=' + encodeURIComponent(segment.method || '') +
