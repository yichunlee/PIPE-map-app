...
async function refreshWgisFileList() {
    ...
    try {
        const res = await fetch(API_URL + '?action=listWgisFiles');
...
async function toggleWgisDataset(fileId, fileName) {
    ...
    if (!ds.loaded) {
        ...
        try {
            const res = await fetch(API_URL + '?action=getWgisFile&fileId=' + encodeURIComponent(fileId));
            const data = await res.json();
            if (!data.success) throw new Error(data.error);
    ...
    reader.onload = async function(e) {
        try {
            ...
            const res = await fetch(API_URL, {
                method: 'POST',
                body: JSON.stringify({ action: 'uploadWgisFile', fileName: file.name, data: base64, email: currentUser ? currentUser.email : '' })
            });
            const data = await res.json();
            if (!data.success) throw new Error(data.error);
...
async function deleteWgisCloudFile(fileId, fileName) {
    ...
    try {
        ...
        const res = await fetch(API_URL + '?action=deleteWgisFile&fileId=' + encodeURIComponent(fileId) + '&email=' + encodeURIComponent(currentUser ? currentUser.email : ''));
        const data = await res.json();
        if (!data.success) throw new Error(data.error);
