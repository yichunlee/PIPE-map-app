...
window.wholeSegmentMarkComplete = async function(segmentNumber) {
    ...
    map.closePopup();
    try {
        const r = await (await fetch(API_URL + '?action=updateWholeSegment&pipelineId=' + encodeURIComponent(currentPipeline.id) + '&segmentNumber=' + encodeURIComponent(segmentNumber) + '&statusString=' + encodeURIComponent(Array(numSmallSegs).fill(sv).join(',')))).json();
...
window.wholeSegmentMarkIncomplete = async function(segmentNumber) {
    ...
    map.closePopup();
    try {
        const r = await (await fetch(API_URL + '?action=updateWholeSegment&pipelineId=' + encodeURIComponent(currentPipeline.id) + '&segmentNumber=' + encodeURIComponent(segmentNumber) + '&statusString=' + encodeURIComponent(Array(numSmallSegs).fill('0').join(',')))).json();
        if (!r.success) throw new Error(r.error || '更新失敗');
        await _refreshProgress();
...
async function _refreshProgress() {
    const d = await (await fetch(API_URL + '?action=getProgress&pipelineId=' + encodeURIComponent(currentPipeline.id))).json();
    if (d.segments) {
...
window.batchMarkComplete = async function() {
    ...
    try {
        ...
        for (const segNum in segmentGroups) {
            ...
            const url = API_URL + 
                '?action=updateWholeSegment' +
                '&pipelineId=' + encodeURIComponent(currentPipeline.id) +
                '&segmentNumber=' + segNum +
                '&statusString=' + encodeURIComponent(statusString);
